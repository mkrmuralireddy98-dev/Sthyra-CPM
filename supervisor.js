...
function sb(path,opts){opts=opts||{};var bearer=authToken||BACKEND.key;opts.headers=Object.assign({apikey:BACKEND.key,Authorization:"Bearer "+bearer},opts.headers||{});return fetch(BACKEND.url+path,opts);}
...
function uploadMedia(file,key){
  var bearer=authToken||BACKEND.key;   // signed-in inspector's JWT; no x-upsert so an upload can't overwrite existing media (SEC-2)
  return fetch(BACKEND.base+"/storage/v1/object/sth-media/"+key,{method:"POST",headers:{apikey:BACKEND.key,Authorization:"Bearer "+bearer,"Content-Type":file.type||"application/octet-stream"},body:file})
    .then(function(r){ if(!r.ok) throw new Error("upload "+r.status); return BACKEND.base+"/storage/v1/object/public/sth-media/"+key; });
}
/* ---- Auth (Supabase) — reads + shared links stay open, writes require a signed-in inspector (SEC-1) ---- */
var authToken=null, authUser=null, sbClient=null;
function applySession(ss){ authToken=ss?ss.access_token:null; authUser=ss?(ss.user||null):null; updateAuthChip(); }
function updateAuthChip(){ var a=$(".avatar"); if(!a) return; if(authUser){ a.classList.add("is-authed"); a.title="Signed in: "+authUser.email+" — tap to sign out"; } else { a.classList.remove("is-authed"); a.title="Sign in to save"; } }
function loadSupabase(cb){ if(sbClient) return cb(sbClient); if(window.__sbLoad){ window.__sbLoad.push(cb); return; } window.__sbLoad=[cb]; var el=document.createElement("script"); el.src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"; el.onload=function(){ try{ sbClient=window.supabase.createClient(BACKEND.base,BACKEND.anonKey,{auth:{persistSession:true,autoRefreshToken:true,storageKey:"sthyra-auth"}}); sbClient.auth.getSession().then(function(r){ applySession(r.data?r.data.session:null); }); sbClient.auth.onAuthStateChange(function(_e,ss){ applySession(ss); }); }catch(e){ try{console.error("[sth] supabase init",e);}catch(_){} } var q=window.__sbLoad||[]; window.__sbLoad=null; q.forEach(function(fn){ try{ fn(sbClient); }catch(_){} }); }; document.head.appendChild(el); }
function signIn(email,pw){ loadSupabase(function(c){ if(!c){ toast("Auth unavailable","warn"); return; } c.auth.signInWithPassword({email:email,password:pw}).then(function(r){ if(r.error){ toast("Sign-in failed: "+r.error.message,"warn"); return; } applySession(r.data.session); closeSheet(); toast("Signed in as "+r.data.user.email); render(); }); }); }
function signOut(){ loadSupabase(function(c){ if(c) c.auth.signOut().then(function(){ applySession(null); toast("Signed out"); render(); }); }); }
function requireAuth(){ if(authToken) return true; toast("Sign in to save changes","warn"); authSheet(); return false; }
function authSheet(){ sheet("Sthyra · Inspector access","Sign in to save", '<div class="muted" style="font-size:12.5px;margin-bottom:10px">Reading and shared links stay open. Saving — captures, notes, calibration — needs a signed-in inspector so every change is attributable.</div><label class="eyebrow">Email</label><input id="auEmail" type="email" value="inspector@sthyra.demo" style="width:100%;padding:11px;border-radius:8px;border:1px solid var(--hair-2);background:var(--paper-2);font-size:13px;margin:6px 0 12px"/><label class="eyebrow">Password</label><input id="auPw" type="password" value="Sthyra@2026" style="width:100%;padding:11px;border-radius:8px;border:1px solid var(--hair-2);background:var(--paper-2);font-size:13px;margin-top:6px"/>', '<button class="btn btn--solid" data-act="do-signin">Sign in</button>'); }
function uid(){return Math.floor(Date.now()).toString(36)+"-"+Math.floor(Math.random()*1e9).toString(36);}
function token(){var a="abcdefghijkmnpqrstuvwxyz23456789ABCDEFGHJKLMNPQRSTUVWXYZ",n=22,b=new Uint8Array(n),s="";(window.crypto||window.msCrypto).getRandomValues(b);for(var i=0;i<n;i++)s+=a[b[i]%a.length];return s;}
...
function onClick(e){
  ...
  switch(a){
    ...
    case "do-signin": signIn(($("#auEmail")||{}).value,($("#auPw")||{}).value); break;
    case "copy-share": try{navigator.clipboard.writeText(act.dataset.link||"");}catch(_){} toast("Link copied"); break;
...
function savePlanPoint(pl,kind,c){if(!requireAuth())return;sbInsert("sth_plan_points",{plan_id:pl.id,kind:kind,px:c.x,py:c.y}).then(function(){return ensurePlanPointsFresh(pl.id);}).then(function(){toast("Control point added");render();});}
...
function doCloseNote(idn){if(!requireAuth())return;sbPatch("sth_notes",idn,{status:"closed"}).then(function(){closeSheet();return loadAll();}).then(function(){toast("Note closed");render();});}
...
function doShare(){
  ...
  sbInsert("sth_shares",{project_id:state.pid,capture_id:cap.id,token:tk,scope:"capture",expires_at:exp}).then(function(){
    ...
    sheet("Share · read-only","Restricted link",
      ...
      '<button class="btn btn--solid" data-act="copy-share" data-link="'+esc(link)+'">'+I.share+' Copy link</button>');
...
function boot(){
  ...
  var av=$(".avatar"); if(av){ av.style.cursor="pointer"; av.addEventListener("click",function(){ authUser?signOut():authSheet(); }); }
