...
(function () {
  ...
  let authToken = null, authUser = null, sbClient = null;
  function sb(path, opts){
    opts = opts || {};
    const bearer = authToken || BACKEND.key;   // signed-in inspector's JWT for writes; publishable key for anon reads
    opts.headers = Object.assign({ apikey: BACKEND.key, Authorization: "Bearer " + bearer }, opts.headers || {});
  ...
  // Single declarative manifest: table name + query + how to apply its rows + realtime flag.
  // Replaces the positional Promise.all -> d[0..28] coupling (ARCH-2 / ARCH-8 / MAINT-1).
  const MANIFEST = {
    project:        { name:"cpm_project", q:"select=*&limit=1", rt:true, apply:r=>{ if(r&&r[0]) Object.assign(PROJECT,{ name:r[0].name, meta:r[0].meta, health:r[0].health, healthDelta:r[0].health_delta }); } },
    kpis:           { name:"cpm_kpis", q:"select=label,val,foot,bar,tone&order=sort", rt:true, apply:r=>replaceArr(KPIS,r) },
    attention:      { name:"cpm_attention", q:"select=tone,t,d,actions&order=sort", rt:true, apply:r=>replaceArr(ATTENTION,r) },
    feed:           { name:"cpm_feed", q:"select=t,txt&order=sort", rt:true, apply:r=>replaceArr(FEED,r) },
    issues:         { name:"cpm_issues", q:"select=id,t,loc,sev,who,status,age,resp,drawing&order=sort", rt:true, apply:r=>replaceArr(ISSUES,r) },
    materials:      { name:"cpm_materials", q:"select=name,unit,stock,days,rate,status,po&order=sort", rt:true, apply:r=>replaceArr(MATERIALS,r) },
    material_alerts:{ name:"cpm_material_alerts", q:"select=tone,t,d,meta,act&order=sort", rt:true, apply:r=>replaceArr(MAT_ALERTS,r) },
    work_packages:  { name:"cpm_work_packages", q:"select=name,code,pct,status,lbl&order=sort", rt:true, apply:r=>replaceArr(WORKPKGS,r) },
    payments:       { name:"cpm_payments", q:"select=contractor,milestone,claim,checks,release,held,amount,note&order=sort", rt:true, apply:r=>replaceArr(PAYMENTS,r) },
    scorecards:     { name:"cpm_scorecards", q:"select=name,trade,score,m&order=sort", rt:true, apply:r=>replaceArr(SCORECARDS,r) },
    labor_teams:    { name:"cpm_labor_teams", q:"select=team,count&order=sort", rt:true, apply:r=>replaceArr(LABOR_TEAMS,(r||[]).map(x=>[x.team,x.count])) },
    agents:         { name:"cpm_agents", q:"select=icon,name,job,live,last&order=sort", rt:true, apply:r=>replaceArr(AGENTS,r) },
    alerts:         { name:"cpm_alerts", q:"select=level,title,detail,agent&order=sort", rt:true, apply:r=>{ const al=r||[]; ALERTS.crit.length=0; ALERTS.warn.length=0; ALERTS.low.length=0; al.forEach(a=>{ if(ALERTS[a.level]) ALERTS[a.level].push([a.title,a.detail,a.agent]); }); } },
    cameras:        { name:"cpm_cameras", q:"select=zone,people,time,bbox,off&order=sort", rt:false, apply:r=>replaceArr(CAMERAS,r) },
    zones:          { name:"cpm_zones", q:"select=id,name,count,x,y,w,h&order=sort", rt:false, apply:r=>replaceArr(ZONES,r) },
    timeline:       { name:"cpm_timeline", q:"select=date,tag,labor,mat,work,issues&order=sort", rt:false, apply:r=>replaceArr(TIMELINE,r) },
    checklist:      { name:"cpm_checklist", q:"select=grp,items&order=sort", rt:false, apply:r=>replaceArr(CHECKLIST,(r||[]).map(x=>({ group:x.grp, items:x.items }))) },
    qr:             { name:"cpm_qr", q:"select=id,name,type,pending,last&order=sort", rt:false, apply:r=>replaceArr(QRLOCS,r) },
    proof:          { name:"cpm_proof", q:"select=stage,state,note,tone,clean&order=sort", rt:false, apply:r=>replaceArr(PROOF,r) },
    defects:        { name:"cpm_defects", q:"select=t,d,conf,box&order=sort", rt:false, apply:r=>replaceArr(DEFECTS,r) },
    pour:           { name:"cpm_pour", q:"select=*&limit=1", rt:false, apply:r=>{ if(r&&r[0]){ const p=r[0]; Object.assign(POUR,{ id:p.pour_id, grade:p.grade, rmc:p.rmc, trucks:p.trucks, batch:p.batch, arrival:p.arrival, start:p.start_t, end:p.end_t, slump:p.slump, cubes:p.cubes, vol:p.vol, weather:p.weather, labor:p.labor, curing:p.curing, approval:p.approval, reminders:p.reminders }); } } },
    mat_flow:       { name:"cpm_mat_flow", q:"select=step&order=sort", rt:false, apply:r=>replaceArr(MAT_FLOW,(r||[]).map(x=>x.step)) },
    draw_mismatch:  { name:"cpm_draw_mismatch", q:"select=t,d,sev&order=sort", rt:false, apply:r=>replaceArr(DRAW_MISMATCH,r) },
    draw_sets:      { name:"cpm_draw_sets", q:"select=name,rng,status&order=sort", rt:false, apply:r=>replaceArr(DRAW_SETS,(r||[]).map(x=>[x.name,x.rng,x.status])) },
    walk_scenes:    { name:"cpm_walk_scenes", q:"select=name,floor,note,draw,flag,mx,my,stage&order=sort", rt:false, apply:r=>replaceArr(WALK_SCENES,r) },
    qc_processes:   { name:"cpm_qc_processes", q:"select=id,sort,name,phase,status,rating,feedback,inspector,checked_at&order=sort", rt:true, apply:r=>replaceArr(QC,r) },
    qc_stages:      { name:"cpm_qc_stages", q:"select=*&order=sort", rt:true, apply:r=>replaceArr(QSTAGES,r) },
    qc_items:       { name:"cpm_qc_items", q:"select=*&order=stage_id,sort", rt:true, apply:r=>replaceArr(QITEMS,r) },
    estimate:       { name:"cpm_estimate", q:"select=stage,family,material,base,unit,wastage,total,source,confidence&order=sort", rt:false, apply:r=>replaceArr(EST,r) }
  };
  async function getTable(m){
    const r = await sb("/" + m.name + "?" + m.q);
    if(!r.ok) throw new Error(m.name + " HTTP " + r.status);
    return r.json();
  }
  // fetch every table; apply each independently so one failure can't blank the rest (PERF-4)
  async function loadBackend(){
    try{
      const keys = Object.keys(MANIFEST);
      const settled = await Promise.allSettled(keys.map(k => getTable(MANIFEST[k])));
      settled.forEach((res, i) => {
        const k = keys[i];
        if(res.status === "fulfilled"){ try{ MANIFEST[k].apply(res.value); }catch(e){ console.error("[cpm] apply " + k + " failed", e); } }
        else console.error("[cpm] load " + MANIFEST[k].name + " failed:", (res.reason && res.reason.message) || res.reason);
      });
      backendLive = settled.some(res => res.status === "fulfilled");
      return backendLive;
    }catch(e){ console.error("[cpm] loadBackend failed", e); return false; }
  }
  // refetch ONLY the changed table (was: full 29-table reload on every realtime event) — PERF-1 / SCALE-2
  async function refetchTable(tableName){
    const k = Object.keys(MANIFEST).filter(key => MANIFEST[key].name === tableName)[0];
    if(!k) return;
    try{ MANIFEST[k].apply(await getTable(MANIFEST[k])); }
    catch(e){ console.error("[cpm] refetch " + tableName + " failed", e); }
  }
  ...
  function onRealtime(payload){
    const t = payload && payload.table; if(!t) return;
    rtPending[t] = true;                          // coalesce the tables touched within the debounce window
    if(rtTimer) clearTimeout(rtTimer);
    rtTimer = setTimeout(function(){
      const tables = Object.keys(rtPending); rtPending = {};
      Promise.all(tables.map(refetchTable)).then(function(){ if(state.view !== "walk") render(); setFoot("update"); });
    }, 350);
  }
  /* ---- Auth (Supabase) — reads stay open, writes require a signed-in inspector (SEC-1/SEC-5) ---- */
  function applySession(session){
    authToken = session ? session.access_token : null;
    authUser  = session ? (session.user || null) : null;
    updateAuthChip();
  }
  function updateAuthChip(){
    const a = $(".avatar"); if(!a) return;
    if(authUser){ a.classList.add("is-authed"); a.title = "Signed in: " + authUser.email + " — click to sign out"; }
    else { a.classList.remove("is-authed"); a.title = "Sign in to approve & save"; }
  }
  // load supabase-js once; one client serves both auth (session) and realtime
  function loadSupabase(cb){
    if(sbClient) return cb(sbClient);
    if(window.__sbLoad){ window.__sbLoad.push(cb); return; }
    window.__sbLoad = [cb];
    const el = document.createElement("script");
    el.src = "https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";
    el.onload = function(){
      try{
        sbClient = window.supabase.createClient(BACKEND.base, BACKEND.anonKey, { auth:{ persistSession:true, autoRefreshToken:true, storageKey:"sthyra-auth" } });
        sbClient.auth.getSession().then(function(r){ applySession(r.data ? r.data.session : null); });
        sbClient.auth.onAuthStateChange(function(_e, session){ applySession(session); });
      }catch(e){ console.error("[cpm] supabase init failed", e); }
      const q = window.__sbLoad || []; window.__sbLoad = null;
      q.forEach(function(fn){ try{ fn(sbClient); }catch(_){} });
    };
    document.head.appendChild(el);
  }
  function signIn(email, pw){
    loadSupabase(function(c){
      if(!c){ notify("Auth unavailable", "crit"); return; }
      c.auth.signInWithPassword({ email:email, password:pw }).then(function(r){
        if(r.error){ notify("Sign-in failed: " + r.error.message, "crit"); return; }
        applySession(r.data.session); closeDrawer(); notify("Signed in as " + r.data.user.email, "ok"); render();
      });
    });
  }
  function signOut(){
    loadSupabase(function(c){ if(c) c.auth.signOut().then(function(){ applySession(null); notify("Signed out", "warn"); render(); }); });
  }
  function requireAuth(){
    if(authToken) return true;
    notify("Sign in to save changes", "warn");
    authDrawer();
    return false;
  }
  function authDrawer(){
    openDrawer(
      '<div class="drawer__head"><div><div class="drawer__eyebrow">Sthyra · Inspector access</div><div class="drawer__title">Sign in to save</div></div><button class="drawer__close" data-act="close">×</button></div>'+
      '<div class="drawer__body">'+
        '<p class="muted" style="font-size:13px;line-height:1.6;margin:0 0 16px">Reading the dashboard is open. Saving — approvals, issue resolution, quality verdicts — requires a signed-in inspector so every change is attributable.</p>'+
        '<div class="subhead">Email</div><input id="authEmail" class="auth-input" type="email" value="inspector@sthyra.demo" autocomplete="username">'+
        '<div class="subhead">Password</div><input id="authPw" class="auth-input" type="password" value="Sthyra@2026" autocomplete="current-password">'+
      '</div>'+
      '<div class="drawer__foot"><button class="ghostbtn" data-act="close">Cancel</button><button class="solidbtn" data-act="signin">Sign in</button></div>'
    );
  }
  function startRealtime(){
    if(window.__cpmRT) return;
    window.__cpmRT = true;
    loadSupabase(function(client){
      if(!client) return;
      try{
        rtClient = client;
        const ch = client.channel("cpm-live");
        Object.keys(MANIFEST).filter(function(k){ return MANIFEST[k].rt; }).forEach(function(k){
          ch.on("postgres_changes", { event:"*", schema:"public", table:MANIFEST[k].name }, onRealtime);
        });
  ...
  function bind(){
    ...
    const av = $(".avatar"); if(av){ av.style.cursor = "pointer"; av.addEventListener("click", function(){ authUser ? signOut() : authDrawer(); }); }
    $("#drawer").addEventListener("click", e=>{
      if(e.target.closest("[data-act='close']")){ closeDrawer(); return; }
      if(e.target.closest("[data-act='signin']")){ signIn(($("#authEmail")||{}).value, ($("#authPw")||{}).value); return; }
